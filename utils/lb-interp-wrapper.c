...
#define PROGNAME	"lb-interp-wrapper"

enum interp_type {
	INTERP_SHELL,
};

static int (*lb_execvep)(const char *file, char *const argv[], char *const envp[]);
static char **origenv;
static char linkdir[] = "/tmp/lb-interp-wrapper-XXXXXX";
...
{
	lb_execvep(argv[0], argv, origenv);
...
int main(int argc, char **argv)
{
	...
	lb_execvep = dlsym(NULL, "lb_execvep");
	if (lb_execvep == NULL) {
		fprintf(stderr, "%s (%s): unable to look up lb_execvep symbol "
	...
	putenv("LDBOX_DISABLE_MAPPING=1");
	...
	if (pid == 0) {
		...
		tools = lb__read_string_variable_from_lua__("ldbox_tools_root");
