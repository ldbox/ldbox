/* lb-show:
 * ldbox Mapping rule testing utility
 *
 ...
 * 			liblb.so; if lb-show is executed outside of
 *			an lb session, liblb.so is not available.
*/

#include "liblbcallers.h"

/* create call_lbshow__binary_type__() */
LIBLB_CALLER(char *, lbshow__binary_type__,
	(const char *filename), (filename),
	NULL)

/* create call_lbshow__map_path2__() */
LIBLB_CALLER(char *, lbshow__map_path2__,
...
/* create call_lbshow__reverse_path__() */
LIBLB_CALLER(char *, lbshow__reverse_path__,
	(const char *func_name, \
        const char *abs_path, uint32_t classmask),
	(func_name, abs_path, classmask),
	NULL)

/* create call_lbshow__execve_mods__() */
LIBLB_CALLER(int, lbshow__execve_mods__,
...
/* create call_lb__set_active_exec_policy_name__() */
LIBLB_VOID_CALLER(lb__set_active_exec_policy_name__,
	(const char *name), (name))

/* create call_lb__lua_c_interface_version__() */
LIBLB_CALLER(const char *, lb__lua_c_interface_version__,
	(void), (), NULL)

/* create call_lbshow__get_real_cwd__() */
LIBLB_CALLER(char *, lbshow__get_real_cwd__,
	(const char *binary_name, const char *fn_name),
	(binary_name, fn_name),
	NULL)

/* create call_lblog_vprintf_line_to_logfile() */
LIBLB_VOID_CALLER(lblog_vprintf_line_to_logfile,
	(const char *file, int line,
        int level, const char *format, va_list ap),
	(file, line, level, format, ap))

/* create call_lb__read_string_variable_from_lua__() */
LIBLB_CALLER(char *, lb__read_string_variable_from_lua__,
	(const char *name), (name), NULL)

/* create call_lbshow__map_network_addr__() */
LIBLB_CALLER(int, lbshow__map_network_addr__,
	(const char *binary_name, const char *fn_name,
...
/* need to have a copy of lblog_printf_line_to_logfile() here;
 * variable arguments are not compatible with the LIBLB_CALLER()
	...
	...)
{
	...
	if (lb_loglevel__ == LB_LOGLEVEL_uninitialized) return;

	va_start(ap, format);
	call_lblog_vprintf_line_to_logfile(file, line, level, format, ap);
	va_end(ap);
}
	...
	const command_table_t *cmds)
{
	...
	fprintf(stderr, "\n"
	    "'%s' must be executed inside lb sandbox (see the 'lb' "
	    "command)\n", progname);
...
static int cmd_var(const command_table_t *cmdp, const cmdline_options_t *opts,
			int cmd_argc, char *cmd_argv[])
{
	char *value = call_lb__read_string_variable_from_lua__(cmd_argv[1]);
			...
			int cmd_argc, char *cmd_argv[])
{
	...
	if (!strcmp(cmd_argv[1], "addr")) {
		...
		res = call_lbshow__map_network_addr__(opts->binary_name,
	...
	void *priv)
{
	...
	/* fix __LB_BINARYNAME in the environment that is going to be
	 * given to lbshow__execve_mods__() (otherwise it would be "lb-show"
	*/
	if (asprintf(&ba[0], "__LB_BINARYNAME=%s", binary_name) < 0) {
	...
	if (call_lbshow__execve_mods__(argv[0], argv, orig_env,
		...
		*/
		mapped_path = call_lbshow__map_path2__(binary_name,
			...
			int cmd_argc, char *cmd_argv[])
{
	...
	while (*cmd_argv) {
		char *reversed_path;
		/* lbshow__binary_type__() operates on
		 * real paths; map the path first.. */
		reversed_path = call_lbshow__reverse_path__(
			...
			int cmd_argc, char *cmd_argv[])
{
	...
	while (*cmd_argv) {
		/* lbshow__binary_type__() operates on
		 * real paths; map the path first.. */
		mapped_path = call_lbshow__map_path2__(opts->binary_name, "",
			opts->function_name, *cmd_argv, &readonly_flag);
		if (!mapped_path) {
			printf("%s: Mapping failed\n", *cmd_argv);
		} else {
			char *type = call_lbshow__binary_type__(mapped_path);
			if (opts->opt_verbose) {
				printf("%s: %s\n", mapped_path, type);
			} else {
				printf("%s\n", type);
			...
			int cmd_argc, char *cmd_argv[])
{
	...
	(void)cmd_argc;
	(void)cmd_argv;
	real_cwd_path = call_lbshow__get_real_cwd__(opts->binary_name, opts->function_name);
			...
			int cmd_argc, char *cmd_argv[])
{
	...
	(void)cmd_argc;
	(void)opts;
	LB_LOG(LB_LOGLEVEL_ERROR, "%s", cmd_argv[1]);
	return(0);
}
			...
			int cmd_argc, char *cmd_argv[])
{
	...
	(void)cmd_argc;
	(void)opts;
	LB_LOG(LB_LOGLEVEL_WARNING, "%s", cmd_argv[1]);
	return(0);
}
			...
			int cmd_argc, char *cmd_argv[])
{
	...
	(void)cmd_argv;
	(void)opts;
	printf("%s\n", call_lb__lua_c_interface_version__());
	return(0);
}
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "hm:f:b:Dvtg:x:X:E:p:")) != -1) {
		 ...
		 * when Lua was removed from liblb. Think twice
	...
	if (cmdp->cmd_must_have_session) {
		...
		if (liblb_handle) {
			...
			if (opts.opt_verbose) {
				printf("%s: liblb FOUND\n", opts.progname);
			}
			int *loglevelptr = NULL;
			loglevelptr = dlsym(liblb_handle, "lb_loglevel__");
			if (loglevelptr) {
				lb_loglevel__ = *loglevelptr;
			} else {
				lb_loglevel__ = 0;
			}
		} else {
			if (opts.opt_verbose) {
				printf("%s: liblb was not found\n", opts.progname);
			}
			lb_loglevel__ = 0;
		}
		/* enable mapping */
		unsetenv("LDBOX_DISABLE_MAPPING");

		if (!liblb_handle)
			usage_exit(opts.progname, "This command can only be used "
				"inside a session (e.g. 'lb lb-show ...')", 1, commands);
	}

	if (liblb_handle && active_exec_policy_name) {
		call_lb__set_active_exec_policy_name__(active_exec_policy_name);
	}
