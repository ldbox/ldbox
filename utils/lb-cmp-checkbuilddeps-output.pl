...
# replacement for ldbox's old dpkg-checkbuilddeps wrapper (which
...
my $ldbox_dir = $ENV{'HOME'}."/.ldbox";

my $verbose_messages_env_var=$ENV{'LDBOX_CHECKBUILDDEPS_VERBOSE'};
my $ldbox_target=$ENV{'LDBOX_TARGET'};
if (! defined $ldbox_target) {
	die "LDBOX_TARGET not set";
}
...
sub check_gcc_dependency {
	...
	if (-f "$ldbox_dir/$ldbox_target/lb.config.d/gcc.config.sh") {
			...
			if ($debug) {
                ...
                # toolchains
		my $gcc_conf_file;
                foreach $gcc_conf_file (<$ldbox_dir/$ldbox_target/lb.config.d/gcc*.config.sh>) {
			if ($debug) {
			...
			if (-f $gcc_conf_file) {
				...
				my $line;
				while ($line = <GCC_CONF>) {
					if ($line =~ m/LDBOX_CROSS_GCC_SHORTVERSION="(.*)"/) {
						$short_vrs = $1;
					} elsif ($line =~ m/LDBOX_CROSS_GCC_VERSION="(.*)"/) {
						$long_vrs = $1;
					}
				}
				close GCC_CONF;
	...
	if ($debug) {
		print "\t$required_gcc: ldbox doesn't have it\n";
	}
	return(0);
}
...
foreach $n (@target_listed) {
	...
	} elsif (provided_by_lb($n,$r_target_missing->{$n})) {
		if ($verbose_messages) {
			print "\t$n: provided by ldbox.\n";
		}
...
foreach $n (@both_required) {
	if (defined $r_host_missing->{$n}) {
		...
		my $prev_mode=$ENV{'LDBOX_SESSION_MODE'};
		$ENV{'LDBOX_SESSION_MODE'}="tools";
		my $tools_stat=`dpkg-query -W -f=\'\${Version}\' $n 2>&1`;
		$ENV{'LDBOX_SESSION_MODE'}=$prev_mode;
		chomp($tools_stat);
