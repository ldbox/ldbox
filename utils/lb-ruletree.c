...
void lblog_printf_line_to_logfile(const char *file, int line,
        int level, const char *format,...)
{
	va_list ap;
...
char *ldbox_session_dir = NULL; /* Fake var, referenced by the library=>must have something*/
...
static void dump_rules(ruletree_object_offset_t offs, int indent)
{
	...
	if (rule->rtree_fsr_func_class) {
		...
		if (rule->rtree_fsr_func_class & LB_INTERFACE_CLASS_OPEN)
		       printf("open ");
		if (rule->rtree_fsr_func_class & LB_INTERFACE_CLASS_STAT)
		       printf("stat ");
		if (rule->rtree_fsr_func_class & LB_INTERFACE_CLASS_EXEC)
		       printf("exec ");
		if (rule->rtree_fsr_func_class & LB_INTERFACE_CLASS_SOCKADDR)
		       printf("sockaddr ");
		if (rule->rtree_fsr_func_class & LB_INTERFACE_CLASS_FTSOPEN)
		       printf("ftsopen ");
		if (rule->rtree_fsr_func_class & LB_INTERFACE_CLASS_GLOB)
		       printf("glob ");
		if (rule->rtree_fsr_func_class & LB_INTERFACE_CLASS_GETCWD)
		       printf("getcwd ");
		if (rule->rtree_fsr_func_class & LB_INTERFACE_CLASS_REALPATH)
		       printf("realpath ");
		if (rule->rtree_fsr_func_class & LB_INTERFACE_CLASS_L10N)
		       printf("l10n ");
		if (rule->rtree_fsr_func_class & LB_INTERFACE_CLASS_MKNOD)
		       printf("mknod ");
		if (rule->rtree_fsr_func_class & LB_INTERFACE_CLASS_RENAME)
		       printf("rename ");
		if (rule->rtree_fsr_func_class & LB_INTERFACE_CLASS_SYMLINK)
		       printf("symlink ");
		if (rule->rtree_fsr_func_class & LB_INTERFACE_CLASS_CREAT)
		       printf("creat ");
		if (rule->rtree_fsr_func_class & LB_INTERFACE_CLASS_PROC_FS_OP)
		       printf("proc_fs_op ");
		if (rule->rtree_fsr_func_class & LB_INTERFACE_CLASS_SET_TIMES)
		       printf("set_times ");
		if (rule->rtree_fsr_func_class & LB_INTERFACE_CLASS_REMOVE)
		       printf("remove ");
	...
	case LB_RULETREE_FSRULE_ACTION_CONDITIONAL_ACTIONS:
		...
		break;
	case LB_RULETREE_FSRULE_ACTION_SUBTREE:
		if (print_ruletree_offsets) {
			printf("subtree => %d\n",
				rule->rtree_fsr_rule_list_link);
		} else {
	...
	case LB_RULETREE_FSRULE_ACTION_IF_EXISTS_THEN_REPLACE_BY:
		printf("if_exists_then_replace_by '%s'\n",
			offset_to_ruletree_string_ptr(rule->rtree_fsr_action_offs, NULL));
		break;
	case LB_RULETREE_FSRULE_ACTION_IF_EXISTS_IN:
		printf("if_exists_in '%s'\n",
			offset_to_ruletree_string_ptr(rule->rtree_fsr_action_offs, NULL));
...
static void dump_exec_selection_rules(ruletree_object_offset_t offs, int indent)
{
	ruletree_exec_policy_selection_rule_t	*rule = offset_to_ruletree_object_ptr(
		offs, LB_RULETREE_OBJECT_TYPE_EXEC_SEL_RULE);
...
static void dump_net_rules(ruletree_object_offset_t offs, int indent)
{
	...
	case LB_RULETREE_NET_RULETYPE_ALLOW:
		...
		break;
	case LB_RULETREE_NET_RULETYPE_RULES:
	...
	for (i = 0; i < list_size; i++) {
		...
		if (item_offs) {
			...
			if (hdr) {
				...
				case LB_RULETREE_OBJECT_TYPE_OBJECTLIST:
					print_indent(indent+1);
					printf("List:\n");
					dump_objectlist(item_offs, indent+2);
					break;
				case LB_RULETREE_OBJECT_TYPE_FSRULE:
					print_indent(indent+1);
					printf("FS rule:\n");
					dump_rules(item_offs, indent+2);
					break;
				case LB_RULETREE_OBJECT_TYPE_EXEC_PP_RULE:
					dump_exec_pp_rules(item_offs, indent+1);
					break;
				case LB_RULETREE_OBJECT_TYPE_EXEC_SEL_RULE:
					dump_exec_selection_rules(item_offs, indent+1);
					break;
				case LB_RULETREE_OBJECT_TYPE_NET_RULE:
					dump_net_rules(item_offs, indent+1);
					break;
				case LB_RULETREE_OBJECT_TYPE_STRING:
					print_indent(indent+1);
					printf("STRING ");
	...
	int depth, int *maxdepth, int *nodes)
{
	...
	hdr = offset_to_ruletree_object_ptr(
		tree_offs, LB_RULETREE_OBJECT_TYPE_BINTREE);
...
static void print_ruletree_object_type(ruletree_object_offset_t obj_offs)
{
	...
	if (hdr) {
		...
		switch (hdr->rtree_obj_type) {
		case LB_RULETREE_OBJECT_TYPE_FILEHDR:
			printf("FILEHDR");
			break;
		case LB_RULETREE_OBJECT_TYPE_CATALOG:
			printf("CATALOG");
			break;
		case LB_RULETREE_OBJECT_TYPE_FSRULE:
			printf("FSRULE");
		...
		case LB_RULETREE_OBJECT_TYPE_OBJECTLIST:
			printf("LIST");
			break;
		case LB_RULETREE_OBJECT_TYPE_BINTREE:
			printf("BINTREE");
			break;
		case LB_RULETREE_OBJECT_TYPE_INODESTAT:
			{
			...
			break;
		case LB_RULETREE_OBJECT_TYPE_UINT32:
			...
			break;
		case LB_RULETREE_OBJECT_TYPE_BOOLEAN:
...
static void dump_catalog(ruletree_object_offset_t catalog_offs, const char *catalog_name, int indent)
{
	...
	for (catp = catalog; catp;
		...
		offset_to_ruletree_object_ptr(catp->rtree_cat_next_entry_offs,
			LB_RULETREE_OBJECT_TYPE_CATALOG) : NULL) {
...
int main(int argc, char *argv[])
{
	...
	session_dir = getenv("LDBOX_SESSION_DIR");
	if (!session_dir) {
		fprintf(stderr, "ERROR: no session "
			"(LDBOX_SESSION_DIR is not set) - this program"
