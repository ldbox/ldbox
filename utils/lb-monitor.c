/* lb-monitor:
 *
 * This is used to wrap the commands started by the "lb" script,
			...
			if (fl) {
				...
				if (len < sizeof(varbuf)) {
					if(len > 0) {
						 ...
						 * conflict with ldbox's requirements for
						 * these variables.
						*/
						if (!strcmp(ent->d_name, ld_preload) ||
							...
							if (asprintf(&prefixed_var, "__LB_%s",
								varbuf) >= 0) {
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "L:x:X:dhe:gG:")) != -1) {
		switch (opt) {
		case 'L': ldbox_liblb = optarg; break;
	...
	case 0: /* child - the worker process */
		 ...
		 * will be running in ldbox'ed environment (depending on
		 * the mapping mode & rules, it might not be possible
		 * to execute 'lb-monitor' in that environment)
		*/
		if (ldbox_liblb) {
			...
			if (old_ld_preload) {
				char	*p_liblb = strstr(old_ld_preload,
					ldbox_liblb);

				DEBUG_MSG("child: LD_PRELOAD was '%s'\n",
						old_ld_preload);
				if (!p_liblb) {
					/* LD_PRELOAD is defined, but liblb
					 * was not included. Add it now. */
					if (asprintf(&new_ld_preload, "%s:%s",
						old_ld_preload, ldbox_liblb) < 0) {
						fprintf(stderr,
							 "%s: asprintf failed\n",
							 progname);
						exit(1);
					}
				} /* else liblb seems to be already included */
			} else {
				/* LD_PRELOAD was not set. */
				DEBUG_MSG("child: no previous LD_PRELOAD\n");
				new_ld_preload = ldbox_liblb;
			}
			if (new_ld_preload) {
	...
	*/
	 ...
	 * a process group (for example, if "lb" was started from a shell,
	...
	*/
	...
	if (command_to_exec_at_end_conditionally) {
		/* the command, which is usually lb-exitreport,
		 * won't be executed if all of these are true:
		 *  - session is persistent
		 *  - log file is empty (or does not exist)
		 *  - process accounting was not activated.
		*/
		const char *session_dir = getenv("LDBOX_SESSION_DIR");
		const char *logfile = getenv("LDBOX_MAPPING_LOGFILE");
		const char *acct_data = getenv("LDBOX_COLLECT_ACCT_DATA");
