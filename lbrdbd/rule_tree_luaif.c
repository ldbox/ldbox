...
static int lua_lb_attach_ruletree(lua_State *l)
{
	int attach_result = -1;
	...
	if (ldbox_session_dir) {
		...
		assert(asprintf(&rule_tree_path, "%s/RuleTree.bin", ldbox_session_dir) > 0);
		attach_result = attach_ruletree(rule_tree_path, 1/*keep open*/);
		LB_LOG(LB_LOGLEVEL_DEBUG, "attach(%s) = %d", rule_tree_path, attach_result);
		free(rule_tree_path);
	}
	lua_pushnumber(l, attach_result);
	LB_LOG(LB_LOGLEVEL_DEBUG, "lua_lb_attach_ruletree => %d",
		attach_result);
	return(1);
}

/* "lb.add_rule_to_ruletree(...)
*/
static int lua_lb_add_rule_to_ruletree(lua_State *l)
{
	int	n = lua_gettop(l);
	...
	if (n == 12) {
		...
		LB_LOG(LB_LOGLEVEL_NOISE,
			"lua_lb_add_rule_to_ruletree '%s' => %d",
...
*/
static int lua_lb_add_exec_preprocessing_rule_to_ruletree(lua_State *l)
{
	int	n = lua_gettop(l);
	uint32_t rule_location = 0;
	...
	if (n == 8) {
		...
		LB_LOG(LB_LOGLEVEL_NOISE,
			"lua_lb_add_exec_preprocessing_rule_to_ruletree '%s' => %d",
...
static int lua_lb_ruletree_objectlist_create_list(lua_State *l)
{
	int				n = lua_gettop(l);
	ruletree_object_offset_t	list_offs = 0;

	LB_LOG(LB_LOGLEVEL_NOISE,
		"lua_lb_ruletree_objectlist_create_list n=%d", n);
	if (n == 1) {
		uint32_t	list_size = lua_tointeger(l, 1);
		list_offs = ruletree_objectlist_create_list(list_size);
	}
	LB_LOG(LB_LOGLEVEL_NOISE,
		"lua_lb_ruletree_objectlist_create_list => %d", list_offs);
	lua_pushnumber(l, list_offs);
	return 1;
}

static int lua_lb_ruletree_objectlist_set_item(lua_State *l)
{
	int	n = lua_gettop(l);
	int	res = 0;
	...
	LB_LOG(LB_LOGLEVEL_NOISE,
		"lua_lb_ruletree_objectlist_set_item => %d", res);
	return 0;
}

static int lua_lb_ruletree_objectlist_get_item(lua_State *l)
{
	int				n = lua_gettop(l);
	...
	LB_LOG(LB_LOGLEVEL_NOISE,
		"lua_lb_ruletree_objectlist_get_item => %d", value);
	lua_pushnumber(l, value);
	return 1;
}

static int lua_lb_ruletree_objectlist_get_list_size(lua_State *l)
{
	int		n = lua_gettop(l);
	uint32_t	size = 0;
	...
	LB_LOG(LB_LOGLEVEL_NOISE,
		"lua_lb_ruletree_objectlist_get_list_size => %d", size);
	lua_pushnumber(l, size);
	return 1;
}

static int lua_lb_ruletree_catalog_get(lua_State *l)
{
	int				n = lua_gettop(l);
	ruletree_object_offset_t	value = 0;
	...
	LB_LOG(LB_LOGLEVEL_NOISE,
		"lua_lb_ruletree_catalog_get => %d", (int)value);
	lua_pushnumber(l, value);
	return 1;
}

static int lua_lb_ruletree_catalog_get_uint32(lua_State *l)
{
	int		n = lua_gettop(l);
	uint32_t	res = 0;
	...
	LB_LOG(LB_LOGLEVEL_NOISE,
		"lua_lb_ruletree_catalog_get_uint32 @%u => %u", offs, res);
	lua_pushnumber(l, res);
	return 1;
}

static int lua_lb_ruletree_catalog_get_boolean(lua_State *l)
{
	int		n = lua_gettop(l);
	uint32_t	res = 0;
	ruletree_object_offset_t	offs = 0;
	...
	LB_LOG(LB_LOGLEVEL_NOISE,
		"lua_lb_ruletree_catalog_get_boolean @%u => %u", offs, res);
	lua_pushboolean(l, res);
	return 1;
}

static int lua_lb_ruletree_catalog_get_string(lua_State *l)
{
	int		n = lua_gettop(l);
	ruletree_object_offset_t	offs = 0;
	const char	*str = NULL;
	...
	LB_LOG(LB_LOGLEVEL_NOISE,
		"lua_lb_ruletree_catalog_get_string @%u => %s", offs, str);
	lua_pushstring(l, str);
	return 1;
}


static int lua_lb_ruletree_catalog_set(lua_State *l)
{
	int	n = lua_gettop(l);
	int	status = 0;
	...
	if (n == 3) {
		...
		LB_LOG(LB_LOGLEVEL_NOISE,
			"lua_lb_ruletree_catalog_set(%s,%s,%d) => %d",
			catalog_name, object_name, value_offset, status);
	} else {
		LB_LOG(LB_LOGLEVEL_NOISE,
			"lua_lb_ruletree_catalog_set => %d", status);
	}
	lua_pushnumber(l, status);
	return 1;
}

static int lua_lb_ruletree_catalog_vget(lua_State *l)
{
	int				n = lua_gettop(l);
	ruletree_object_offset_t	value = 0;
	...
	LB_LOG(LB_LOGLEVEL_NOISE, "%s => %d", __func__, (int)value);
	lua_pushnumber(l, value);
	return 1;
}

static int lua_lb_ruletree_catalog_vset(lua_State *l)
{
	int	n = lua_gettop(l);
	int	status = 0;
	...
	LB_LOG(LB_LOGLEVEL_NOISE, "%s => %d", __func__, status);
	lua_pushnumber(l, status);
	return 1;
}

static int lua_lb_ruletree_new_string(lua_State *l)
{
	int	n = lua_gettop(l);
	...
	if (n == 1) {
		...
		if (str) {
			...
			LB_LOG(LB_LOGLEVEL_NOISE,
				"lua_lb_ruletree_new_string(%s) => %d", str, str_offs);
		} else {
			LB_LOG(LB_LOGLEVEL_NOISE,
				"lua_lb_ruletree_new_string(NULL) => %d", str_offs);
		}
	} else {
		LB_LOG(LB_LOGLEVEL_NOISE,
			"lua_lb_ruletree_new_string => %d", str_offs);
	}
	lua_pushnumber(l, str_offs);
	return 1;
}

static int lua_lb_ruletree_new_uint32(lua_State *l)
{
	int	n = lua_gettop(l);
...
static int lua_lb_ruletree_new_boolean(lua_State *l)
{
	int	n = lua_gettop(l);
	ruletree_object_offset_t	ui32_offs = 0;
...
static int lua_lb_add_exec_policy_selection_rule_to_ruletree(lua_State *l)
{
	int	n = lua_gettop(l);
	uint32_t rule_location = 0;
...
static int lua_lb_add_net_rule_to_ruletree(lua_State *l)
{
	int	n = lua_gettop(l);
	uint32_t rule_location = 0;
	...
	*/
	if (n == 11) {
		...
		LB_LOG(LB_LOGLEVEL_NOISE, "%s:", __func__);
		...
		rule.rtree_net_log_level = lblog_level_name_to_number(lua_tostring(l, 8));
		...
		if (errno_str) {
			...
			if (!strcmp(errno_str, "ENETUNREACH")) e = ENETUNREACH;
				...
				LB_LOG(LB_LOGLEVEL_NOISE, "errno: use default (EACCESS)");
				e = EACCES; /* the default */
			}
			LB_LOG(LB_LOGLEVEL_NOISE, "errno = %s => #%d", errno_str, e);
...
{
	{"objectlist_create",		lua_lb_ruletree_objectlist_create_list},
	{"objectlist_set",		lua_lb_ruletree_objectlist_set_item},
	{"objectlist_get",		lua_lb_ruletree_objectlist_get_item},
	{"objectlist_size",		lua_lb_ruletree_objectlist_get_list_size},

	/* simple interface to catalogs (two levels) */
	{"catalog_get",			lua_lb_ruletree_catalog_get},
	{"catalog_set",			lua_lb_ruletree_catalog_set},
	{"catalog_get_uint32",		lua_lb_ruletree_catalog_get_uint32},
	{"catalog_get_boolean",		lua_lb_ruletree_catalog_get_boolean},
	{"catalog_get_string",		lua_lb_ruletree_catalog_get_string},

	/* alternative interface to catalogs (variable levels) */
	{"catalog_vget",		lua_lb_ruletree_catalog_vget},
	{"catalog_vset",		lua_lb_ruletree_catalog_vset},
	...
	*/
	{"new_string",			lua_lb_ruletree_new_string},
	{"new_uint32",			lua_lb_ruletree_new_uint32},
	{"new_boolean",			lua_lb_ruletree_new_boolean},

	{"attach_ruletree",		lua_lb_attach_ruletree},

	/* FS rules */
	{"add_rule_to_ruletree",	lua_lb_add_rule_to_ruletree},

	/* exec rules */
	{"add_exec_preprocessing_rule_to_ruletree",	lua_lb_add_exec_preprocessing_rule_to_ruletree},
	{"add_exec_policy_selection_rule_to_ruletree",	lua_lb_add_exec_policy_selection_rule_to_ruletree},

	/* Network rules */
	{"add_net_rule_to_ruletree",	lua_lb_add_net_rule_to_ruletree},
