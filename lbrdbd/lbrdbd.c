...
lua_State *lbrdbd_lua;

static void load_and_execute_lua_file(const char *filename)
{
	const char *errmsg;

	switch(luaL_loadfile(lbrdbd_lua, filename)) {
	case LUA_ERRFILE:
		fprintf(stderr, "Error loading %s\n", filename);
		exit(1);
	case LUA_ERRSYNTAX:
		errmsg = lua_tostring(lbrdbd_lua, -1);
...
{
	fprintf(stderr,
		"lbrdbd: Lua interpreter PANIC: unprotected error in call to Lua API (%s)\n",
		lua_tostring(l, -1));
#if 0
...
static char *read_string_variable_from_lua(const char *name)
{
	...
	if (name && *name) {
		lua_getglobal(lbrdbd_lua, name);
		result = (char *)lua_tostring(lbrdbd_lua, -1);
		if (result) {
			result = strdup(result);
		}
		lua_pop(lbrdbd_lua, 1);
		LB_LOG(LB_LOGLEVEL_DEBUG,
			"Lua variable %s = '%s', gettop=%d",
			name, (result ? result : "<NULL>"),
			lua_gettop(lbrdbd_lua));
	}
	return(result);
}
...
static void initialize_lua(void)
{
	...
	lbrdbd_lua = luaL_newstate();
	lua_atpanic(lbrdbd_lua, lb_lua_panic);

	luaL_openlibs(lbrdbd_lua);
#if 0
	lua_bind_lb_functions(lbrdbd_lua); /* register our lb_ functions */
#endif
	lua_bind_ruletree_functions(lbrdbd_lua); /* register our ruletree_ functions */
	lua_bind_lblib_functions(lbrdbd_lua); /* register our lblib.* functions */

	load_and_execute_lua_file(main_lua_script);

	/* check Lua/C interface version. */
	lua_if_version = read_string_variable_from_lua("lbrdbd_lua_c_interface_version");
	if (!lua_if_version) {
		LB_LOG(LB_LOGLEVEL_ERROR, "FATAL ERROR: "
			"init.lua script didn't provide"
			" 'lbrdbd_lua_c_interface_version' identifier!");
		exit(1);
	}
	if (strcmp(lua_if_version, LBRDBD_LUA_C_INTERFACE_VERSION)) {
		LB_LOG(LB_LOGLEVEL_ERROR, "FATAL ERROR: "
			"init.lua script interface version mismatch:"
			" script has '%s', but '%s' was expected",
			lua_if_version, LBRDBD_LUA_C_INTERFACE_VERSION);
		exit(1);
	}
	free(lua_if_version);

	LB_LOG(LB_LOGLEVEL_INFO, "lua initialized.");
	LB_LOG(LB_LOGLEVEL_NOISE, "gettop=%d", lua_gettop(lbrdbd_lua));
...
char *execute_init2_script(void)
{
	...
	LB_LOG(LB_LOGLEVEL_INFO, "Loading '%s'", init2_script);
	...
	if (!result) {
		LB_LOG(LB_LOGLEVEL_ERROR, "init2 scripts didn't provide"
			" 'init2_result'!");
	}
	return(result);
}
...
int main(int argc, char *argv[])
{
	 ...
	 * that LB startup fails if the OS is changed someday
	...
	*/
	...
	lblog_init_level_logfile_format(debug_level, debug_file, NULL);
	...
	LB_LOG(LB_LOGLEVEL_DEBUG, "Initializing");
	...
	if (create_ruletree_file(rule_tree_path,
		...
		LB_LOG(LB_LOGLEVEL_ERROR, "Failed to create rule tree file (%s)",
			rule_tree_path);
		fprintf(stderr, "Failed to create rule DB!\n");
		exit(1);
	}
	LB_LOG(LB_LOGLEVEL_DEBUG, "Rule tree file opened & mapped to memory");
	...
	if (start_server) {
		...
		LB_LOG(LB_LOGLEVEL_DEBUG, "Initializing server");
		...
		if (backgroud_server) {
			if ((worker_pid = fork()) != 0) {
				/* parent */
				LB_LOG(LB_LOGLEVEL_DEBUG, "Parent process");
