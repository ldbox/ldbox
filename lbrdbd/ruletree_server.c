	...
	ruletree_rpc_msg_reply_t *reply)
{
	...
	LB_LOG(LB_LOGLEVEL_DEBUG, "clearfileinfo dev=%lld ino=%lld",
	...
	if (ruletree_find_inodestat(&handle, &istat_in_db) < 0) {
		/* not found. */
		LB_LOG(LB_LOGLEVEL_DEBUG, "clearfileinfo: not found");
	} else {
		/* found - update */

		LB_LOG(LB_LOGLEVEL_DEBUG, "clearfileinfo: found");
	...
	ruletree_rpc_msg_reply_t *reply)
{
	...
	LB_LOG(LB_LOGLEVEL_DEBUG, "setfileinfo dev=%lld ino=%lld",
	...
	if (ruletree_find_inodestat(&handle, &istat_in_db) < 0) {
		/* not found. */
		LB_LOG(LB_LOGLEVEL_DEBUG, "setfileinfo: not found, set");
		...
		LB_LOG(LB_LOGLEVEL_DEBUG, "setfileinfo: found, update");
		if (command->rim_message.rimm_fileinfo.inodesimu_active_fields &
			...
			LB_LOG(LB_LOGLEVEL_DEBUG, "setfileinfo: found, set uid to %d",
		...
		if (command->rim_message.rimm_fileinfo.inodesimu_active_fields &
			...
			LB_LOG(LB_LOGLEVEL_DEBUG, "setfileinfo: found, set gid to %d",
		...
		if (command->rim_message.rimm_fileinfo.inodesimu_active_fields &
			...
			LB_LOG(LB_LOGLEVEL_DEBUG, "setfileinfo: found, set device: 0%o, 0x%X",
	...
	ruletree_rpc_msg_reply_t *reply)
{
	...
	istat_in_db = command->rim_message.rimm_fileinfo;
	LB_LOG(LB_LOGLEVEL_DEBUG, "releasefileinfo dev=%lld ino=%lld",
		(long long)istat_in_db.inodesimu_dev,
		(long long)istat_in_db.inodesimu_ino);
	...
	if (ruletree_find_inodestat(&handle, &istat_in_db) < 0) {
		...
		LB_LOG(LB_LOGLEVEL_DEBUG, "releasefileinfo: not found");
		reply->hdr.rimr_message_type = RULETREE_RPC_MESSAGE_REPLY__OK;
	} else {
		/* found - update */
		uint32_t prev_active_fields = istat_in_db.inodesimu_active_fields;
		...
		if (command->rim_message.rimm_fileinfo.inodesimu_active_fields) {
			...
			LB_LOG(LB_LOGLEVEL_DEBUG, "releasefileinfo: found, update");
			if (command->rim_message.rimm_fileinfo.inodesimu_active_fields &
			    RULETREE_INODESTAT_SIM_UID) {
				LB_LOG(LB_LOGLEVEL_DEBUG, "releasefileinfo: release uid");
				istat_in_db.inodesimu_active_fields &= ~RULETREE_INODESTAT_SIM_UID;
			}
			if (command->rim_message.rimm_fileinfo.inodesimu_active_fields &
			    RULETREE_INODESTAT_SIM_GID) {
				LB_LOG(LB_LOGLEVEL_DEBUG, "releasefileinfo: release gid");
				istat_in_db.inodesimu_active_fields &= ~RULETREE_INODESTAT_SIM_GID;
			}
			if (command->rim_message.rimm_fileinfo.inodesimu_active_fields &
			    RULETREE_INODESTAT_SIM_MODE) {
				LB_LOG(LB_LOGLEVEL_DEBUG, "releasefileinfo: found, release mode");
				istat_in_db.inodesimu_active_fields &= ~RULETREE_INODESTAT_SIM_MODE;
			}
			if (command->rim_message.rimm_fileinfo.inodesimu_active_fields &
			    RULETREE_INODESTAT_SIM_DEVNODE) {
				LB_LOG(LB_LOGLEVEL_DEBUG, "releasefileinfo: found, release device node");
				istat_in_db.inodesimu_active_fields &= ~RULETREE_INODESTAT_SIM_DEVNODE;
			}
		}
		ruletree_set_inodestat(&handle, &istat_in_db);
...
void ruletree_server(void)
{
	...
	LB_LOG(LB_LOGLEVEL_DEBUG, "Entering server loop");
	while (1) {
		int	r;
		size_t	reply_size = sizeof(ruletree_rpc_msg_reply_hdr_t);

		LB_LOG(LB_LOGLEVEL_DEBUG, "get message");
