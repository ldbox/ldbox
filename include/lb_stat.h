...
extern int lb_stat_file(const char *path, struct stat *buf, int *result_errno_ptr,
	int (*statfn_with_ver_ptr)(int ver, const char *filename, struct stat *buf),
	int ver,
	int (*statfn_ptr)(const char *filename, struct stat *buf));

extern int lb_stat64_file(const char *path, struct stat64 *buf, int *result_errno_ptr,
...
extern int lb_fstat(int fd, struct stat *statbuf);
