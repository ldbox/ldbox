...
/* This file contains the "exec core" of ldbox; Being able to alter
 * how the execl(), execve(), etc. exec-class functions are handled 
 * is one of the most important features of ldbox.
 *
 ...
 *    preload/liblb.c. Eventually, the gate function will call "do_exec()"
 ...
 *    call the regular path mapping engine of ldbox to get a real path to
 ...
 *           be started directly (for example, the lb-show command that
 *           belongs to ldbox's utilities)
 ...
 *    do_exec(), which will call lb_next_execve(). It transfers control to
 ...
 * "lb-show" application, so that "lb-show" can print out how the
	...
	if (elf_hdr_match(region, HOST_ELF_MACHINE_32, HOST_ELF_DATA)) {
		...
		for (i = 0; i < eh->e_phnum; i++) {
			...
			if (ph->p_type == PT_INTERP) {
				LB_LOG(LB_LOGLEVEL_DEBUG, "%s: 32-bit ELF, PT_INTERP='%s'",
					__func__, region + ph->p_offset);
				info->pt_interp = strdup(region + ph->p_offset);
				return (BIN_HOST_DYNAMIC);
			}
		}
		LB_LOG(LB_LOGLEVEL_DEBUG, "%s: 32-bit ELF, static", __func__);
	...
	if (elf_hdr_match(region, HOST_ELF_MACHINE_64, HOST_ELF_DATA)) {
		...
		for (i = 0; i < eh->e_phnum; i++) {
			ph = (Elf64_Phdr *)
			    (region + eh->e_phoff + (i * ph_entsize));
			if (ph->p_type == PT_INTERP) {
				LB_LOG(LB_LOGLEVEL_DEBUG, "%s: 64-bit ELF, PT_INTERP='%s'",
					__func__, region + ph->p_offset);
				info->pt_interp = strdup(region + ph->p_offset);
				return (BIN_HOST_DYNAMIC);
			}
		}
		LB_LOG(LB_LOGLEVEL_DEBUG, "%s: 64-bit ELF, static", __func__);
		return (BIN_HOST_STATIC);
	}
#endif
	...
	struct binary_info *info)
{
	...
	LB_LOG(LB_LOGLEVEL_DEBUG, "%s(%s)", __func__, filename);
		...
		if (sb1_bug_emulation_mode && 
			...
			*/
			if (access_nomap_nolog(filename, R_OK) < 0) {
				saved_errno = errno;
				LB_LOG(LB_LOGLEVEL_DEBUG, "no X or R "
			...
			*/
			LB_LOG(LB_LOGLEVEL_DEBUG, "no X permission for '%s'",
				filename);
			retval = BIN_INVALID;
			errno = saved_errno;
			goto _out;
	 ...
	 */
	if (!target_cpu) {
		LB_LOG(LB_LOGLEVEL_DEBUG, "Lookin up target_cpu..");
		target_cpu = ruletree_catalog_get_string("config", "ldbox_cpu");
		if (!target_cpu) {
			target_cpu = "arm";
			LB_LOG(LB_LOGLEVEL_DEBUG,
	...
	const char *exec_policy_name)
{
	...
	char **new_argv = NULL;
	char hashbang[LDBOX_MAXPATH]; /* only 60 needed on linux, just be safe */
	char interpreter[LDBOX_MAXPATH];
	...
	if ((c = read(fd, &hashbang[0], LDBOX_MAXPATH - 1)) < 2) {
	...
	/* Now we need to update __LB_ORIG_BINARYNAME to point to 
	...
	*/
	change_environment_variable(
		*envpp, "__LB_ORIG_BINARYNAME=", interpreter);
	...
	case 2:
		...
		if (mapped_interpreter) free(mapped_interpreter);
                mapped_interpreter = NULL;
                {
                        ...
                        ldbox_map_path_for_exec("script_interp",
		...
		break;
	case -1:
		LB_LOG(LB_LOGLEVEL_DEBUG, "%s: <-1> exec denied", __func__);
		if (mapped_interpreter) free(mapped_interpreter);
		mapped_interpreter = NULL;
		return(-1);
	default:
	...
	change_environment_variable(*envpp, "__LB_BINARYNAME=",
	    mapped_binaryname);
	free(mapped_binaryname);
	free(tmp);
	
	LB_LOG(LB_LOGLEVEL_DEBUG, "prepare_hashbang(): interpreter=%s,"
	...
	LB_LOG(LB_LOGLEVEL_DEBUG, "prepare_hashbang done: mapped_file='%s'",
			*mapped_file);
...
static char **duplicate_argv(char *const *argv)
{
	...
	LB_LOG(LB_LOGLEVEL_NOISE2, "duplicate_argv: argc=%d", argc);
	my_argv = (char **)calloc(argc + 1, sizeof(char *));
	for (i = 0, p = (char **)argv; *p; p++) {
		my_argv[i++] = strdup(*p);
		LB_LOG(LB_LOGLEVEL_NOISE2, "duplicate_argv: [%d] = '%s'", i-1, my_argv[i-1]);
	}
	my_argv[i] = NULL;
 ...
 * pieces of exec*() processing: This will add/check ldbox's
 * private variables and rename some user-specified
 * variables:
 * - LD_LIBRARY_PATH is expected to be the user's version.
 *   contents of it will be moved to __LB_LD_LIBRARY_PATH.
 ...
 * - LDBOX_SESSION_DIR can not be changed or removed
 * - LDBOX_MAPPING_METHOD can not be changed, if it has been set.
 *
 * N.B later steps (postprocessing) will set LD_LIBRARY_PATH and
 *    LD_PRELOAD to the actual values that should be active
 *    during the real exec; lb's initialization code will
 *    restore the __LB__LD... variables to the real variables.
	...
	const char *binaryname, char *const *envp)
{
	...
	int	has_ldbox_session_dir = 0;
	int	has_ldbox_session_mode = 0;
	int     has_ldbox_sigtrap = 0;
	int	has_ldbox_mapping_method = 0;
	const int ldbox_session_dir_varname_len = strlen("LDBOX_SESSION_DIR");
	const int ldbox_sigtrap_varname_len = strlen("LDBOX_SIGTRAP");
	const int ldbox_mapping_method_varname_len = strlen("LDBOX_MAPPING_METHOD");
	const int ldbox_session_varname_prefix_len = strlen("LDBOX_SESSION_");
	const int ldbox_vperm_ids_varname_len = strlen("LDBOX_VPERM_IDS");
	const int ldbox_vperm_request_varname_len = strlen("LDBOX_VPERM_REQUEST");
	const char *user_vperm_request = NULL;

	/* LDBOX_SESSION_* is now preserved properly (these are practically
	...
	*/
	...
	/* if the caller sets LDBOX_SIGTRAP, then assume that it was
	 * intentional and leave it alone, but if they just call
	 * execve without LDBOX_SIGTRAP in the environment, then
	 * LDBOX_SIGTRAP should be inherited from us
	 */

	/* count the environment variables and arguments, also check
	 * for LD_PRELOAD, LD_LIBRARY_PATH, LDBOX_SESSION_* and
	 * LDBOX_SIGTRAP (and LDBOX_MAPPING_METHOD)
	 */
		...
		case 'L':
			if (strncmp("LD_PRELOAD=", *p, strlen("LD_PRELOAD=")) == 0) {
				if (asprintf(&user_ld_preload,
					"__LB_%s", *p) < 0) {
					LB_LOG(LB_LOGLEVEL_ERROR,
						"asprintf failed to create __LB_%s", *p);
				}
				continue;
			}
			...
			break;
		case 'S':
			if (strncmp("LDBOX_SESSION_DIR=", *p,
			     ldbox_session_dir_varname_len+1) == 0) {
				has_ldbox_session_dir = 1;
				if (strcmp(*p+ldbox_session_dir_varname_len+1,
					ldbox_session_dir)) {
						LB_LOG(LB_LOGLEVEL_WARNING,
			...
			if (ldbox_mapping_method &&
			    (strncmp("LDBOX_MAPPING_METHOD=", *p,
			      ldbox_mapping_method_varname_len+1) == 0)) {
				has_ldbox_mapping_method = 1;
				if (strcmp(*p+ldbox_mapping_method_varname_len+1,
					ldbox_mapping_method)) {
						LB_LOG(LB_LOGLEVEL_WARNING,
							"Detected attempt to set %s,"
							" restored to %s",
							*p, ldbox_mapping_method);
				}
				continue;
			}
			if (strncmp("LDBOX_SIGTRAP=", *p,
			     ldbox_sigtrap_varname_len+1) == 0) {
				has_ldbox_sigtrap = 1;
				continue;
			}
			break;
	 ...
	 * Just restore those. */
	if (!has_ldbox_session_dir) {
		LB_LOG(LB_LOGLEVEL_NOTICE,
			"Detected attempt to clear LDBOX_SESSION_DIR, "
				"restored to %s", ldbox_session_dir);
	}
	if (ldbox_mapping_method && !has_ldbox_mapping_method) {
		LB_LOG(LB_LOGLEVEL_NOTICE,
			"Detected attempt to clear LDBOX_MAPPING_METHOD, "
				"restored to %s", ldbox_mapping_method);
	}
	...
	for (i = 0, p=(char **)envp; *p; p++) {
		if (strncmp(*p, "__LB_", strlen("__LB_")) == 0) {
			/* __LB_* are temporary variables that must not
			 * be relayed to the next executable => skip it.
			 * Such variables include: __LB_BINARYNAME,
			 * __LB_REAL_BINARYNAME, __LB_ORIG_BINARYNAME,
			 * __LB_CHROOT_PATH
			*/
			continue;
		}
		...
		case 'S':
			if (strncmp(*p, "LDBOX_SESSION_MODE=",
					ldbox_session_varname_prefix_len+5) == 0) {
				/* user-provided LDBOX_SESSION_MODE */
				char *requested_mode = *p +
					ldbox_session_varname_prefix_len+5;
				char *rulefile = NULL;

				if (ldbox_session_mode &&
				    (strcmp(requested_mode, ldbox_session_mode) == 0)) {
					/* same as current mode - skip it */
					continue;
				}

				if (asprintf(&rulefile, "%s/rules/%s.lua",
					ldbox_session_dir, requested_mode) < 0) {
				...
				if (access_nomap_nolog(rulefile, R_OK) == 0) {
					LB_LOG(LB_LOGLEVEL_DEBUG,
						...
						requested_mode);
					has_ldbox_session_mode = 1;
				}
				free(rulefile);
				if (has_ldbox_session_mode == 0) continue;
			} else if (strncmp(*p, "LDBOX_SESSION_",
					ldbox_session_varname_prefix_len) == 0) {
				/* this is user-provided LDBOX_SESSION_*, skip it. */
				continue;
			} else if (ldbox_mapping_method &&
				   (strncmp(*p, "LDBOX_MAPPING_METHOD=",
					ldbox_mapping_method_varname_len+1) == 0)) {
				/* this is user-provided LDBOX_MAPPING_METHOD,
				 * but we already have ldbox_mapping_method. Skip it. */
				continue;
			} else if (strncmp(*p, "LDBOX_VPERM_REQUEST=",
					ldbox_vperm_request_varname_len+1) == 0) {
				user_vperm_request = *p;
				/* remember it, but don't pass to the started program.*/
				continue;
			} else if (strncmp(*p, "LDBOX_VPERM_IDS=",
					ldbox_vperm_ids_varname_len+1) == 0) {
				/* user-provided or old LDBOX_VPERM_IDS, skip it */
				continue;
			}
			break;
		}
	...
	if (asprintf(&(my_envp[i]), "LDBOX_SESSION_DIR=%s", ldbox_session_dir) < 0) {
		LB_LOG(LB_LOGLEVEL_ERROR,
			"asprintf failed to create LDBOX_SESSION_DIR");
	} else {
		i++;
	}
	...
	if (ldbox_mapping_method) {
		if (asprintf(&(my_envp[i]), "LDBOX_MAPPING_METHOD=%s", ldbox_mapping_method) < 0) {
			LB_LOG(LB_LOGLEVEL_ERROR,
				"asprintf failed to create LDBOX_MAPPING_METHOD");
		} else {
			i++;
		}
	}
	...
	if (ldbox_session_mode && (has_ldbox_session_mode==0)) {
		if (asprintf(&(my_envp[i]), "LDBOX_SESSION_MODE=%s",
		     ldbox_session_mode) < 0) {
			LB_LOG(LB_LOGLEVEL_ERROR,
				"asprintf failed to create LDBOX_SESSION_MODE");
		} else {
			i++;
		}
	}
	...
	my_envp[i] = vperm_export_ids_as_string_for_exec("LDBOX_VPERM_IDS=", 0,0,0, user_vperm_request);
	if (!my_envp[i]) {
		LB_LOG(LB_LOGLEVEL_ERROR,
			"vperm_export_ids_as_string to create LDBOX_VPERM_IDS");
	} else {
		i++;
	}

	/* add back LDBOX_SIGTRAP if it was removed accidentally, so
	 * exec following in GDB will work */
	if (!has_ldbox_sigtrap && getenv("LDBOX_SIGTRAP")) {
		LB_LOG(LB_LOGLEVEL_NOTICE,
		       "Detected attempt to clear LDBOX_SIGTRAP, "
		       "restored to %s", getenv("LDBOX_SIGTRAP"));
		if (asprintf(&(my_envp[i]), "LDBOX_SIGTRAP=%s",
			     getenv("LDBOX_SIGTRAP")) < 0) {
			LB_LOG(LB_LOGLEVEL_ERROR,
			       "asprintf failed to create LDBOX_SIGTRAP");
		} else {
			i++;
		}
	}

	/* __LB_BINARYNAME is used to communicate the binary name
	 * to the new process so that it's available even before
	 * its main function is called
	 */
	if (asprintf(&new_binaryname_var, "__LB_BINARYNAME=%s", binaryname) < 0) {
		LB_LOG(LB_LOGLEVEL_ERROR,
			"asprintf failed to create __LB_BINARYNAME");
	}
	my_envp[i++] = new_binaryname_var; /* add the new process' name */

	if (asprintf(&new_orig_file_var, "__LB_ORIG_BINARYNAME=%s", orig_file) < 0) {
		LB_LOG(LB_LOGLEVEL_ERROR,
			"asprintf failed to create __LB_ORIG_BINARYNAME");
	}
	my_envp[i++] = new_orig_file_var; /* add the new process' name */

	/* __LB_EXEC_BINARYNAME is the original filename; for scripts,
	 * it is the name of script, otherwise it is same as
	 *  __LB_ORIG_BINARYNAME
	*/
	if (asprintf(&new_exec_file_var, "__LB_EXEC_BINARYNAME=%s", orig_file) < 0) {
		LB_LOG(LB_LOGLEVEL_ERROR,
			"asprintf failed to create __LB_EXEC_BINARYNAME");
	}
	my_envp[i++] = new_exec_file_var; /* add the new process' name */

	/* allocate slot for __LB_REAL_BINARYNAME that is filled later on */
	my_envp[i++] = strdup("__LB_REAL_BINARYNAME=");

	/* add user's versions of LD_PRELOAD and LD_LIBRARY_PATH */
	if (user_ld_preload != NULL) {
		my_envp[i++] = user_ld_preload;
		LB_LOG(LB_LOGLEVEL_NOISE, "Added %s", user_ld_preload);
	}
	if (user_ld_library_path != NULL) {
		my_envp[i++] = user_ld_library_path;
		LB_LOG(LB_LOGLEVEL_NOISE, "Added %s", user_ld_library_path);
	}

	if (ldbox_chroot_path) {
		/* chroot simulation is active, relay the value */
		if (asprintf(&new_exec_file_var, "__LB_CHROOT_PATH=%s", ldbox_chroot_path) < 0) {
			LB_LOG(LB_LOGLEVEL_ERROR,
				"asprintf failed to create __LB_CHROOT_PATH");
		}
 ...
 * a *lot* of noise if LB_LOGLEVEL_NOISE is enabled.
*/
	...
	char *const *orig_strv, char *const *new_strv) 
{
	...
	if (strv_modified) {
		LB_LOG(LB_LOGLEVEL_DEBUG, "%s[] was modified", vecname);
	...
	const char *prefix, size_t *index)
{
	...
	LB_LOG(LB_LOGLEVEL_NOISE2, "strvec_contains_prefix: not found");
	return (0);
}
	...
	char **my_envp, const char *var_prefix, const char *new_value)
{
	...
	if (strvec_contains_prefix(my_envp, var_prefix, &idx)) {
		...
		LB_LOG(LB_LOGLEVEL_DEBUG, "Changed: %s", new_value_buf);
	} else {
		LB_LOG(LB_LOGLEVEL_DEBUG, "Failed to change %s%s",
			var_prefix, new_value);
	}
}
	...
	int host_compatible_binary)
{
	if (info->mode & (S_ISUID | S_ISGID)) {
		...
		change_environment_variable(my_envp, "LDBOX_VPERM_IDS=", new_vperm_str);
		...
		if (host_compatible_binary) {
			if ((info->mode & S_ISUID)) {
				LB_LOG(LB_LOGLEVEL_WARNING,
					"SUID bit set for '%s' (ldbox may be disabled)",
					filename);
			}

			if ((info->mode & S_ISGID)) {
				LB_LOG(LB_LOGLEVEL_WARNING,
					"SGID bit set for '%s' (ldbox may be disabled)",
					filename);
			}
		}
	} else {
		LB_LOG(LB_LOGLEVEL_DEBUG, "not SUID/SGID");
	}
}
	...
	const char *cputransp_name, const struct binary_info *info)
{
	...
	LB_LOG(LB_LOGLEVEL_DEBUG, "exec_policy: %s, exec_flags: %u",
		exec_policy_name, exec_flags);

	if (!(exec_flags & LB_EXEC_FLAGS_FORCE_CPU_TRANSPARENCY))
		return 0;
	...
	if (cputransp_cmd && cputransp_cmd[0]) {
		...
		*/
		...
		if (binary_can_run_on(info, cputransp_arch)) {
			LB_LOG(LB_LOGLEVEL_DEBUG, "forcing CPU transparency");
			return 1;
		}
	}
	...
	char ***new_envp) /* *new_envp must be filled by the caller */
{
	...
	START_PROCESSCLOCK(LB_LOGLEVEL_INFO, &clk1, "prepare_exec");
	...
	if (!file_has_been_mapped) {
		...
		START_PROCESSCLOCK(LB_LOGLEVEL_INFO, &clk2, "execve_preprocess");
		if ((err = apply_exec_preprocessing_rules(&my_file, &my_argv, &my_envp)) != 0) {
			LB_LOG(LB_LOGLEVEL_ERROR, "argvenvp processing error %i", err);
		}
		STOP_AND_REPORT_PROCESSCLOCK(LB_LOGLEVEL_INFO, &clk2, my_file);
	}
	...
	*/
	if (file_has_been_mapped) {
	...
	} else if (strvec_contains_prefix(my_envp, "LDBOX_DISABLE_MAPPING=1", NULL)) {
		LB_LOG(LB_LOGLEVEL_DEBUG,
			"do_exec(): mapping disabled, my_file = %s", my_file);
		mapped_file = strdup(my_file);

		/* we won't call ldbox_map_path_for_exec() because mapping
		 * is disabled.  */
	} else {
		...
		clear_mapping_results_struct(&mapping_result);
		START_PROCESSCLOCK(LB_LOGLEVEL_INFO, &clk3, "map_path_for_exec");
		ldbox_map_path_for_exec("do_exec", my_file, &mapping_result);
		...
		STOP_AND_REPORT_PROCESSCLOCK(LB_LOGLEVEL_INFO, &clk3, mapped_file);
	...
	change_environment_variable(my_envp,
		"__LB_REAL_BINARYNAME=", mapped_file);
	...
	LB_LOG(LB_LOGLEVEL_DEBUG, "%s: exec_policy_name=%s", __func__, exec_policy_name);

	START_PROCESSCLOCK(LB_LOGLEVEL_INFO, &clk4, "exec/typeswitch");
	switch (type) {
		case BIN_HASHBANG:
			LB_LOG(LB_LOGLEVEL_DEBUG, "Exec/hashbang %s", mapped_file);
		...
		case BIN_HOST_DYNAMIC:
			LB_LOG(LB_LOGLEVEL_DEBUG, "Exec/host-dynamic %s",
					mapped_file);

#if 0
			postprocess_result = lb_execve_postprocess("native",
				exec_policy_name,
		...
		case BIN_HOST_STATIC:
#if 0
			postprocess_result = lb_execve_postprocess("static",
				exec_policy_name,
				&mapped_file, &my_file, binaryname,
				&my_argv, &my_envp);
#else
			if (exec_policy_force_cpu_transparency(exec_policy_name, "native", &info)) {
				...
				*/
				allow_static_bin = getenv("LDBOX_ALLOW_STATIC_BINARY");
				if (allow_static_bin &&
				 ...
				 * hope of getting back to LB. It won't
		...
		case BIN_TARGET:
			LB_LOG(LB_LOGLEVEL_DEBUG, "Exec/target %s",
					mapped_file);

#if 0
			postprocess_result = lb_execve_postprocess(
				"cpu_transparency",
				exec_policy_name,
				&mapped_file, &my_file,
	...
	STOP_AND_REPORT_PROCESSCLOCK(LB_LOGLEVEL_INFO, &clk4, my_file);
    out:
	*new_file = mapped_file;
	*new_argv = my_argv;
	*new_envp = my_envp;
	err = errno;
	STOP_AND_REPORT_PROCESSCLOCK(LB_LOGLEVEL_INFO, &clk1, orig_file);
	...
	char *const *orig_argv, char *const *orig_envp)
{
	...
	START_PROCESSCLOCK(LB_LOGLEVEL_INFO, &clk1, "do_exec");
	if (getenv("LDBOX_DISABLE_MAPPING")) {
		...
		if (r < 0) {
			*result_errno_ptr = errno;
			...
			STOP_AND_REPORT_PROCESSCLOCK(LB_LOGLEVEL_INFO, &clk1, "Exec denied");
		...
		if (check_envp_has_ld_preload_and_ld_library_path(
			...
			*result_errno_ptr = EINVAL;
			STOP_AND_REPORT_PROCESSCLOCK(LB_LOGLEVEL_INFO, &clk1, "Config error");
			return(-1);
		}
	}
	...
	char **new_file, char ***new_argv, char ***new_envp)
{
	...
	LB_LOG(LB_LOGLEVEL_DEBUG, "%s '%s'", __func__, orig_argv[0]);
	...
	*new_envp = prepare_envp_for_do_exec(file, binaryname, orig_envp);
	...
	ret = prepare_exec("lbshow_exec", NULL/*exec_policy_name*/,
 ...
 * buffer.
*/
char *lbshow__binary_type__(const char *filename)
{
