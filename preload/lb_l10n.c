/*
 * lb_l10n.c -- exec_policy based localization support.
 *
	...
	const char *realfn_name, const char *domainname, const char *dirname)
{
	...
	if (dirname != NULL) { 
		ldbox_map_path(realfn_name, dirname,
		     LDBOX_MAP_PATH_ALLOW_NONEXISTENT, &res,
		     LB_INTERFACE_CLASS_L10N);
		assert(res.mres_result_path != NULL);

		mapped_dirname = res.mres_result_path;

		LB_LOG(LB_LOGLEVEL_DEBUG, "binding domain %s to %s",
	...
	*result_errno_ptr = errno;
	...
	if (result && (*result != '\0')) {
		...
		ldbox_path = scratchbox_reverse_path(realfn_name, result,
				LB_INTERFACE_CLASS_L10N);
		if (ldbox_path) {
			/* FIXME: this will leak memory. nobody will free
			 * the buffer at *ldbox_path; in fact the manual
